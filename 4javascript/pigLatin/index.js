...
function encodeConsonantWord(word) {
    ...
    if (word[0].match(regex)) {
        ...
        let vowelInd = word.indexOf(word.match(regex)[0])
        result = word.substr(vowelInd) + '-' + word.substr(0, vowelInd) + 'ay'
        word = result
    }
    return word;
}
...
function encodeText(text) {
    ...
    for (let i = 0; i < pigArr.length; i++) {
        pigArr[i] = encodeWord(pigArr[i]);
    }
    return pigArr.join(" ");
        // if (pigArr[i].match(vowel)) {
        //     pigArr[i] = pigArr[i] + 'yay'
        //     console.log(pigArr[i])
        // } else {
        //     let pigCword = pigArr.split(0, 1)
        //     result = text.substr(pigCword) + "-" + text.substr(0, pigCword) + "ay"
        // }
